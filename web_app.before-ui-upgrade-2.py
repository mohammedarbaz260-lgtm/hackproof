...
from auth import db, User, Report
...
def register():
    ...
    <html>
    <head>
        <title>HackProof AI - Register</title>
        <style>
            * { box-sizing: border-box; }
            body {
                margin: 0;
...
def dashboard():
    ...
    report_html = "".join(f'<div class="report-item"><strong>{r.analysis_type}</strong> — {r.target}<br><small>{r.created_at}</small><br><pre>{r.result}</pre></div>' for r in reports)
    return f"""
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <body>
        ...
        <aside class="sidebar">
            ...
            <a href="/dashboard">📚 Knowledge Base</a>
